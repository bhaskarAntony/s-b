...
export const getAvailableRooms = async (req, res) => {
  try {
    ...
    const availableRooms = rooms.filter(room => {
      if (!room._id) {
        console.warn(`Room with roomNumber ${room.roomNumber} has no _id`);
        return false;
      }

      // If room is not booked, it's available
      if (!room.isBooked) {
        return true;
      }

      // If room is booked, check for date overlap
      const roomCheckIn = new Date(room.checkInDate);
      const roomCheckOut = new Date(room.checkOutDate);

      console.log(`Checking availability for room ${room.roomNumber}: ${checkInDate >= roomCheckOut || checkOutDate <= roomCheckIn}`);
      

      // Room is available if requested checkIn is after room's checkOut or requested checkOut is before room's checkIn
      return checkInDate >= roomCheckOut || checkOutDate <= roomCheckIn;
    });
